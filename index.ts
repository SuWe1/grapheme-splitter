...
function shouldBreak(start:any, mid:any, end:any) {
  ...
  if (eModifierIndex > 1 &&
    all.slice(1, eModifierIndex).every(function (c:any) { return c == Extend; }) &&
  ...
  if (previous == CR && next == LF) {
    ...
    if (next == E_Modifier && mid.every(function (c:any) { return c == Extend; })) {
      return Break;
    } else {
      return BreakStart;
    }
...
function getGraphemeBreakProperty(code:any) {
...
export class GraphemeSplitter {
  ...
  public nextBreak = (str:string, index:number) : number => {
    if (index === undefined) {
      index = 0;
    }
    if (index < 0) {
  ...
  public splitGraphemes = (str:string) => {
    const res = [];
    let index = 0;
    let brk = 0;
    const check = () => {
      brk = this.nextBreak(str, index);
      return brk < str.length;
    };
    while (check()) {
  ...
  public iterateGraphemes = (str:string) => {
    let index = 0;
    const res = {
      next: () => {
        let value;
  ...
  public countGraphemes = (str:string) => {
    let count = 0;
    let index = 0;
    let brk = 0;
    const check = () => {
